...
def train_asr(clusterfile,
    ...
    elif database_cfg['train_mode']=='onlynonsupervised':
        train_asr_onlynonsupervised(clusterfile, job_name, task_index, ssh_command, expdir)
    # if training partly nonsupervised but still only validating the prediction of the targets
...
def train_asr_onlynonsupervised(clusterfile,
              job_name,
              task_index,
              ssh_command,
              expdir):
...
####### TEMPORAL ADJUSTMENTS
    ...
    #create the classifier
    # if using a non supervised classifier, the output dimension should be a tuple
    classifier = asr_factory.factory(
        conf=nnet_cfg,
        output_dim = (coder.num_labels,int(quant_cfg['quant_levels'])))
...
def train_asr_nonsupervised(clusterfile,
    ...
    ''' does everything for asr partly nonsupervised training
    Args:
        clusterfile: the file where all the machines in the cluster are
            specified if None, local training will be done
        job_name: one of ps or worker in the case of distributed training
