...
class LstmFeatureReconstructor(reconstructor.Reconstructor):
    ...
    def reconstruct(self, hlfeat, reconstructor_inputs, is_training):
        ...
        reconstructor_inputs = tf.slice(reconstructor_inputs,[0,0,0],[-1,max_nbr_input_features,-1])

        # pad with extra zero features if length is not equal to input_per_hl*max_nbr_features
        ...
        reconstructor_inputs = tf.reshape(reconstructor_inputs,[-1, input_per_hl, input_dim])

        # transform the inputs to a list
        inputs = tf.unstack(reconstructor_inputs, axis=1)

        #Make the initial state
        zero_hidden_state = rnn_cell.zero_state(batch_size*max_nbr_features, tf.float32).h
        ...
        output, _ = tf.contrib.legacy_seq2seq.rnn_decoder(decoder_inputs=inputs,
                        initial_state=init_state, cell=rnn_cell, scope='rnn_decoder')
